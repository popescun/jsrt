...
extern jsrt_sciter::dom_event_handler g_domEventHandler;
...
bool engine_init(struct engine *engine) {
  ...
  r = SciterLoadFile(engine, WSTR("/storage/emulated/0/Android/obb/com.example.jsrt_android/facade/main.htm"));
...
void engine_draw_frame(struct engine *engine) {
  if (engine->display == NULL) {
    // No display.
    LOGI("engine_draw_frame display null");
    return;
  }
  ...
  switch (cmd) {
    ...
    case APP_CMD_INIT_WINDOW:
      ...
      if (engine->app->window != NULL) {
        LOGI("engine_handle_cmd APP_CMD_INIT_WINDOW");
        engine_init(engine);
        engine_init_display(engine);
        engine_draw_frame(engine);
        sciter::attach_dom_event_handler(engine, &g_domEventHandler);
      }
      break;
